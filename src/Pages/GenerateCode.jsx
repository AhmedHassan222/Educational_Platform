...
export default function GenerateCode() {
  ...
  async function addCodes() {
    ...
    try {
      await axios
        .post(`${baseURL}/codes/create?courseId=${courseId}`, formData, {
          headers: {
        ...
        .then((res) => {
          ...
          if (res.data.message === "codes created successfuly") {
            toast.success("قد تم الإنشاء الاكواد  ", {
              position: "top-center",
              autoClose: 3000,
      ...
      if (
        error?.response?.data?.Error[0][0].message === "must After from date "
      ) {
      ...
      <div className="container py-5">
        <div className="text-center rounded-4  border-1 widthCustom mx-auto">
          <form encType="multipart/form-data" onSubmit={handleSubmit}>
            ...
            {/* fromDate */}
            <div className="mb-4">
              <label htmlFor="fromDate" className="py-2 text-end w-100 ">
                البداية
              </label>
              <input
                id="fromDate"
            ...
            {/* toDate */}
            <div className="mb-4">
              <label htmlFor="toDate" className="py-2 text-end w-100 ">
                النهاية
              </label>
              <input
                id="toDate"
              ...
              >
                ...
                {Courses?.map((course, index) => (
                  <option key={index} value={course?._id}>
                    {course?.name}-{stage[course?.subCategoryId?.name]}{" "}
                    {grade[course?.categoryId?.name]}
                  </option>
                ))}
              </select>
              {isSubmit ? (
