...
export default function AddTeacher() {
  ...
  async function addTeacher() {
    ...
    const payload = {
      ...formData,
      phoneNumber: `+20${formData.phoneNumber.slice(1)}`,
    };
    console.log(payload.phoneNumber);
    try {
      await axios
        .post(`${baseURL}/auth/addTeacher?courseId=${CoursesId}`, payload, {
          headers: {
        ...
        .then((response) => {
          console.log(response);
            if (response.data.message === "Done, please try to login" || response.status === 200) {
          ...
          <form onSubmit={submitRegisterForm}>
            ...
            <div className=" mb-4">
              <textarea
                ...
                id="subjecTeacher"
                name="subjecTeacher"
                placeholder="معلومات عن المدرس"
                value={formData.subjecTeacher}
                onChange={handleChange}
              ></textarea>
              {error?.map((err, index) =>
                err.context.label === "subjecTeacher" ? (
                  <div key={index}>
                    {!formData.subjecTeacher ? (
                      <p className="small fw-medium py-2 text-end text-danger">
                        لا يمكن ارسال هذا الحقل فارغا
                      </p>
                    ) : (
