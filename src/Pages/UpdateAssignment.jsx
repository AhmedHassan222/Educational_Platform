...
export default function UpdateAssignment() {
  ...
  async function updateItem() {
    ...
    try {
      await axios
        .patch(`${baseURL}/assignment/update?assignmentId=${id}`, formData, {
          headers: {
  ...
  const handleSubmit = (e) => {
    setIsSubmit(true);
    e.preventDefault();
    if (!assign.title || !assign.desc) return;
    if (!files) return;
    updateItem();
  };
        ...
        <div className="text-center rounded-4  border-1 widthCustom mx-auto">
          <form encType="multipart/form-data" onSubmit={handleSubmit}>
            <div className=" mb-4 text-end">
              {/* pdf */}
              <div className=" mb-4">
                <label className="py-3" htmlFor="filePdf">
                  برجاء اختيار ملف جديدة
                </label>
                <input
                  placeholder=" ادخل ملف"
                  id="filePdf"
                  type="file"
                  className="w-100 p-2"
                  name="filePdf"
                ...
                {isSubmit ? (
                  <>
                    {files ? (
                      !validExtensions.includes(files?.type) ? (
            ...
            >
              {" "}
              {Isloading ? <i className="fa fa-spin fa-spinner"></i> : "حفظ"}
            </button>
          </form>
        </div>
      </div>
