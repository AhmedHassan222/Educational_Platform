...
export default function AddCourse() {
  ...
  async function addItem() {
    ...
    try {
      await axios
        .post(
          `${baseURL}/course/create?subCategoryId=${subcategoryId}`,
          formData,
          {
            headers: {
        ...
        .then((res) => {
          ...
          if (res.data.message === "course created successfuly") {
            toast.success(" قد تم الإضاقة بنجاح", {
              position: "top-center",
              autoClose: 3000,
      ...
      <div className="container py-5">
        ...
        <div className="text-center rounded-4  border-1 widthCustom mx-auto">
          <form encType="multibart/form-data" onSubmit={handleSubmit}>
              ...
              >
                ...
                {subCategoryies?.map((subcategory, index) => (
                  <option key={index} value={subcategory._id}>
                    {stage[subcategory.name]}{" "}
                    {grade[subcategory.categoryId.name]}
                  </option>
                ))}
              </select>
              {isSubmit ? (
            ...
            >
              {Isloading ? <i className="fa-spin fa fa-spinner"></i> : "اضف"}
            </button>
          </form>
        </div>
      </div>
